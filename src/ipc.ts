...
import {Ao3FicAlreadyExists, FfnFicAlreadyExists, addAo3Fic, addFfnFic, getAo3FicDetail, getFfnFicDetail, getLibrary, getMinFic, linkAo3Fic, linkFfnFic, updateAo3Fic, updateFfnFic } from "../shared/Library"
...
ipcMain.handle(IpcEvents.GET_AO3_FIC_URL, (event, library_id, chapter): string => {
...
ipcMain.handle(IpcEvents.ADD_FFNET_FIC_TO_LIBRARY, (event, ficUrl): Promise<number> => {
    const start_time = Date.now();
    const captcha_window = new BrowserWindow({width: 800, height: 600, show: false, webPreferences: {
        preload: path.join(__dirname, "captchaPreload.js"),
    }});
...
ipcMain.handle(IpcEvents.GET_FFNET_FIC_URL, (event, library_id, chapter): string => {
    return new FfnSource().getUrlForChapter(library_id, chapter);
});

ipcMain.handle(IpcEvents.LINK_AO3_FIC, (event, library_id, ficUrl): Promise<number> => {
    return fetch(ficUrl)
        .then(function (response) {
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
        ...
        .then(function(content) {
            ...
            if (exists) {
                console.log("Fic exists and is linked to library_id: ", existinglibid)
                return -1;
            }
...
ipcMain.handle(IpcEvents.LINK_FFNET_FIC, (event, library_id, ficUrl): Promise<number> => {
    const start_time = Date.now();
    const captcha_window = new BrowserWindow({width: 800, height: 600, show: false, webPreferences: {
        preload: path.join(__dirname, "captchaPreload.js"),
    }});
