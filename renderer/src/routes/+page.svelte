...
<p>
	To test our Electron preload script, we are using Node.js v<span id="node-version" />, Chromium
	v{window.versions.chrome()}, and Electron v<span id="electron-version" />.
</p>

<p>
	The <mark>marked</mark> text is inserted through an ipcRenderer.invoke channel. The Chrome version
	is inserted via a variable declared in the preload script.
</p>
